...
function parseFileLine(line: string, runner: string): FileEntry | null {
  ...
  if (runner === 'vitest' || runner === 'unknown') {
    ...
    if (m) return { file: m[2], passed: m[1] === '✓', duration: m[3] }
  }

  if (runner === 'jest') {
    const pass = clean.match(/^PASS\s+(.+?)(?:\s+\([\d.]+\s*s\))?$/)
    if (pass) return { file: pass[1].trim(), passed: true }
    const fail = clean.match(/^FAIL\s+(.+?)(?:\s+\([\d.]+\s*s\))?$/)
    if (fail) return { file: fail[1].trim(), passed: false }
  }

  if (runner === 'pytest') {
    const m = clean.match(/^(PASSED|FAILED)\s+(.+?)\s*$/)
    if (m) return { file: m[2], passed: m[1] === 'PASSED' }
  }
...
export function startCoverageSpinner(label: string, runner = 'unknown'): CoverageSpinner {
  ...
  return {
    onLine: (line) => {
      const entry = parseFileLine(line, runner)
      if (entry) { files.push(entry); render() }
    },
    stop: () => {
      clearInterval(timer)
      if (rendered > 0) { process.stdout.write(`\x1B[${rendered}A\x1B[0J`); rendered = 0 }
      const secs = Math.floor((Date.now() - start) / 1000)
