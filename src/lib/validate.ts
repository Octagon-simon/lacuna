...
export function subjectFromTestPath(testPath: string): string | null {
  ...
  if (/^(index|main|app|setup|utils?|helpers?|constants?|types?)$/i.test(base)) return null
  return base
}
...
export function leakLooksTestFixable(testCode: string, sourceCode: string | null | undefined): boolean {
...
export function detectMocksFileError(errorOutput: string, mocksFiles: string[]): string | null {
...
export function detectJestConfigConflict(rawOutput: string): string | null {
  const m = JEST_MULTIPLE_CONFIGS_RE.exec(rawOutput)
  if (!m) return null
  const sources = m[1].trim().split('\n').map(l => l.replace(/^\s*\*\s*/, '').trim()).filter(Boolean)
  return (
    `Jest found more than one config source and refuses to guess which one to use, so it exited before running any tests (no coverage report was ever written):\n` +
...
export function detectJestValidationError(rawOutput: string): string | null {
  const m = JEST_VALIDATION_ERROR_RE.exec(rawOutput)
  if (!m) return null
  const title = m[1].trim()
  const body = m[2].trim().split('\n').filter(l => !/^\s*at\s/.test(l)).join('\n').trim()
  return (
    `Jest crashed with a fatal "${title}" before running any tests (no coverage report was ever written):\n\n${body}\n\n` +
...
export function detectProcessCrash(rawOutput: string): string | null {
...
export function detectUnrelatedFileCrash(
  errorOutput: string,
  testFilePath: string,
  ...
  const known = [testFilePath, sourceFilePath, ...mocksFiles].filter((p): p is string => Boolean(p))
  const isKnown = (file: string) => known.some(k => file.endsWith(k) || k.endsWith(file))
...
let mocksLock: Promise<void> = Promise.resolve()
export function withMocksLock<T>(fn: () => Promise<T>): Promise<T> {
...
export function countTestCases(code: string): number {
  const stripped = stripNonCode(code)
...
export function countDistinctErrors(output: string): number {
  ...
  if (tsErrors > 0) return TIER_PARSE_ERROR + tsErrors
...
export function buildFailingTestChecklist(rawRunOutput: string): string | null {
  const clean = stripAnsi(rawRunOutput)
  let names = [...new Set(
    [...clean.matchAll(/^\s*●\s+(.+)$/gm)]
      .map(m => m[1].trim())
  ...
  if (names.length === 0) {
    ...
    names = [...new Set(
      [...clean.matchAll(/^\s*FAIL\s+\S+\s*>\s*(.+)$/gm)]
        .map(m => m[1].trim())
        .filter(Boolean),
    )]
  }
  ...
  if (names.length <= LARGE_FAILURE_THRESHOLD) return null
  return (
...
export function extractFailureRegion(output: string, maxChars = 4500): string {
  const clean = stripAnsi(output)
...
export function dedupeMockExports(code: string): string {
  const lines = code.split('\n')
  ...
  if (lastIndexByName.size === blocks.length) return code // no duplicate names — nothing to do
  ...
  blocks.forEach((b, idx) => {
    ...
    if (b.start > cursor) keptLines.push(...lines.slice(cursor, b.start))
    if (lastIndexByName.get(b.name) === idx) keptLines.push(...lines.slice(b.start, b.end))
    cursor = b.end
  })
  if (cursor < lines.length) keptLines.push(...lines.slice(cursor))
...
export function detectUnbalancedMocksSyntax(code: string): boolean {
...
export function fixNeverTypedAsyncMocks(code: string): string {
  if (!/\.mock(?:Resolved|Rejected)Value\(/.test(code)) return code
  const masked = blankStringsAndComments(code)
  ...
  for (const m of matches) {
    if (m.start < last) continue // overlapping match, skip
...
export function buildProcessCrashMessage(crashSignature: string, originalError: string): string {
  return (
    `⚠ CRITICAL — THE TEST PROCESS CRASHED (${crashSignature.slice(0, 150)}). THIS IS NOT A NORMAL TEST REGRESSION.\n\n` +
...
export function buildPatchEscalationMessage(count: number, reason: string): string {
  return (
    `PATCH MODE KEEPS FAILING (${reason}, ${count} times in a row) — SWITCH TO FULL REWRITE MODE.\n` +
...
function normalizePatchMarkerLine(line: string): string {
  return line
    .replace(/^(\s*)(?:<!--|\/\*)\s*(?=@@@)/, '$1// ')
    .replace(/\s*(?:-->|\*\/)\s*$/, '')
}
...
export function parsePatch(patchOutput: string): PatchOperation[] {
  ...
  while (i < lines.length) {
    ...
    if (type === 'REPLACE') {
      ...
      if (!withRe.test(markerLines[i] ?? '')) { i++; continue } // malformed — skip
      i++ // skip // @@@ WITH:
...
export function findCallEnd(code: string, startIdx: number): number {
...
export function detectStrayPatchMarkers(code: string): boolean {
...
export function detectOpenHandleLeak(rawRunOutput: string): boolean {
  return OPEN_HANDLE_LEAK_RE.test(rawRunOutput)
}

export function buildOpenHandleLeakMessage(): string {
  return (
    'Tests passed, but Jest had to force-exit because of a leaked async handle (a setInterval/setTimeout/open ' +
...
export function parseMocksPatch(patchOutput: string): MockPatchOperation[] {
  ...
  while (i < lines.length) {
    ...
    if (type === 'REPLACE') {
      ...
      if (!withRe.test(markerLines[i] ?? '')) { i++; continue }
      i++ // skip // @@@ WITH:
      const newLines: string[] = []
      while (i < lines.length && !endRe.test(markerLines[i])) {
        newLines.push(lines[i])
