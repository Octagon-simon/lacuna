...
import {
  loadCoverage, extractGaps, filterTestableGaps, findUncoveredFiles, findTestFiles, isWithinDir,
} from './coverage/index.js'
import type { CoverageGap, CoverageReport } from './coverage/index.js'
...
export interface ScopeGapsResult {
  ...
  ranSuite: boolean          // whether a coverage command was actually executed
}
 ...
 */
export async function discoverScopeGaps(
  config: LacunaConfig,
  env: DetectedEnvironment,
  cwd: string,
  scopeDir?: string,
  onLine?: (line: string) => void,
): Promise<ScopeGapsResult> {
  ...
  const inScope = (p: string) => !scopeDir || isWithinDir(abs(p), scopeDir)
  ...
  if (hasTests) {
    ...
    if (result.timedOut) {
      throw new DiscoverGapsError(
        `Coverage run timed out after ${config.coverageTimeout}s — a test likely leaks an open handle. ` +
        `Raise coverageTimeout in .lacuna.json.`,
      )
    }
    const configConflict = detectJestConfigConflict(combined)
    if (configConflict) throw new DiscoverGapsError(`Jest never ran any tests — no coverage report exists to read.\n\n${configConflict}`)
    const validationError = detectJestValidationError(combined)
    if (validationError) throw new DiscoverGapsError(validationError)
