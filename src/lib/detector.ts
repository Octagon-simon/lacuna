...
export async function detectEnvironment(
  ...
  try { await readFile(join(cwd, 'Cargo.toml'), 'utf-8'); return RUNNER_DEFAULTS['cargo-test'] } catch { /* not Rust */ }

  try {
    const gemfile = await readFile(join(cwd, 'Gemfile'), 'utf-8')
    if (/\brspec\b/.test(gemfile)) return RUNNER_DEFAULTS.rspec
  } catch { /* not Ruby */ }

  try {
    const composer = await readFile(join(cwd, 'composer.json'), 'utf-8')
    const composerJson = JSON.parse(composer) as { require?: Record<string, string>; 'require-dev'?: Record<string, string> }
    const composerDeps = { ...(composerJson.require ?? {}), ...(composerJson['require-dev'] ?? {}) }
    if ('pestphp/pest' in composerDeps) return RUNNER_DEFAULTS.pest
    if ('phpunit/phpunit' in composerDeps) return RUNNER_DEFAULTS.phpunit
    return RUNNER_DEFAULTS.phpunit // default to phpunit for any PHP project
  } catch { /* not PHP */ }

  try { await readFile(join(cwd, 'Directory.Build.props'), 'utf-8'); return RUNNER_DEFAULTS['dotnet-test'] } catch { /* not .NET */ }

  // Also detect .csproj / .sln as C# indicators
  try {
    const { readdir } = await import('fs/promises')
    const entries = await readdir(cwd)
    if (entries.some(e => e.endsWith('.csproj') || e.endsWith('.sln'))) return RUNNER_DEFAULTS['dotnet-test']
  } catch { /* not C# */ }

  try { await readFile(join(cwd, 'build.gradle'), 'utf-8'); return RUNNER_DEFAULTS['gradle-test'] } catch { /* not Gradle */ }
  try { await readFile(join(cwd, 'build.gradle.kts'), 'utf-8'); return RUNNER_DEFAULTS['gradle-test'] } catch { /* not Gradle Kotlin */ }
  try { await readFile(join(cwd, 'pom.xml'), 'utf-8'); return RUNNER_DEFAULTS['maven-test'] } catch { /* not Maven */ }
  try { await readFile(join(cwd, 'Package.swift'), 'utf-8'); return RUNNER_DEFAULTS['swift-test'] } catch { /* not Swift */ }
