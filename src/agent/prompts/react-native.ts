...
export function buildReactNativeGuidance(): string {
  return [
    ...
    '- PENDING / LOADING-STATE tests: to assert a spinner while an async op is in flight, control the promise with a DEFERRED you resolve yourself — `let resolve; svc.method.mockReturnValue(new Promise(r => { resolve = r }))`. Do NOT use `new Promise(() => {})` (never settles) AND then `await fireEvent.press(...)`/`await act(...)` on the handler that awaits it — the handler never resolves, so the await hangs and the test dies with "Exceeded timeout of 10000 ms". Trigger the action WITHOUT awaiting it, assert the loading indicator (`getByTestId`/`findBy…`), THEN call `resolve(value)` and await the settled UI.',
    '- OUTCOME ASSERTIONS must match the source branch: before asserting `expect(someCallback).toHaveBeenCalledWith(...)` (e.g. showToast/navigate), find that exact call in the component. A SUCCESS path frequently has NO toast — it navigates or opens a modal — while only the CATCH block toasts. Never invent a success message the code does not emit; assert the real outcome of that branch (the modal that opens, the state that changes).',
    ...
    "- WEAK-WAIT RACE (passes locally, fails in CI): NEVER assert hook/component STATE right after a waitFor whose body only checks that a mock was *called*. A mock is called synchronously, before its promise resolves and before the setState that consumes the result runs, so `await waitFor(() => expect(service.getThing).toHaveBeenCalled())` followed by `expect(result.current.items).toEqual(mockItems)` reads the INITIAL value in slow CI. Wait for the work to FINISH instead: put a settle signal inside the waitFor — `await waitFor(() => { expect(service.getThing).toHaveBeenCalled(); expect(result.current.isLoading).toBe(false) })` — then read state. Settle-signal order: isLoading===false, else the asserted value itself inside waitFor, else a terminal error/empty state. Never wait on a signal that is true BEFORE the work finishes (a 'was called' check, or call count 1 when more concurrent calls are coming). EXCEPTION: reading mock.calls[...] arg values after a call-only wait is fine (the call already happened) — only STATE reads need the stronger wait.",
    ...
    '- TOAST VS SCREEN TEXT — check the catch/success block before asserting screen text: if the component calls `showToast(message, ...)` in a catch block, nothing is rendered on screen — assert `expect(showToast).toHaveBeenCalledWith(\'Exact message\', ...)` instead. For error flows use `mockRejectedValue(new Error(\'...\'))`, not `mockResolvedValue({ success: false })`. Do NOT assume the SUCCESS path toasts — many success branches just navigate, open a modal, or update state and call NO toast (only the catch block toasts). Read the actual success branch: assert whatever it really does (the modal that opens, the state that changes); only assert `showToast` on success if that exact call literally exists there. Never write `screen.getByText(\'...\')` for text that goes to a toast service.',
