...
export function analyzeNextJs(sourceCode: string): NextJsAnalysis {
  ...
  for (const m of clientMatches) clientModules.push(m[1])

  const serverModules: string[] = []
  const serverMatches = sourceCode.matchAll(/from\s+['"]([^'"]+\.server)['"]/g)
  for (const m of serverMatches) serverModules.push(m[1])

  const sessionProviders: string[] = []
  const sessionMatches = sourceCode.matchAll(
    /from\s+['"]([^'"]*(?:next-auth|session|auth|useSession|provider)[^'"]*)['"]/gi
  )
  for (const m of sessionMatches) sessionProviders.push(m[1])

  /**
   * CRITICAL FIX:
   * Directives are not "modules", but they *must influence classification*
   */
  const enhancedHasNavigation =
    hasNavigation || hasClientDirective

  const enhancedHasHeaders =
    hasHeaders || hasServerDirective
...
export function buildNextJsGuidance(
  a: NextJsAnalysis,
  mockApi: string
): string | null {
  ...
  if (!isNextJsProject) return null
  ...
  if (a.hasNavigation) {
    lines.push(
      "",
      "NEXT/NAVIGATION MOCK (required):",
      `${mockApi}.mock('next/navigation', () => ({`,
      `  useRouter: ${mockApi}.fn(() => ({ push: ${mockApi}.fn(), replace: ${mockApi}.fn(), back: ${mockApi}.fn() })),`,
      `  usePathname: ${mockApi}.fn(() => '/'),`,
      `  useSearchParams: ${mockApi}.fn(() => ({ get: ${mockApi}.fn(() => null), has: ${mockApi}.fn(() => false) })),`,
      `  useParams: ${mockApi}.fn(() => ({})),`,
      `}))`
    )
  }
  ...
  if (a.hasHeaders) {
    lines.push(
      "",
      "NEXT/HEADERS MOCK (server-only safe stub):",
      `${mockApi}.mock('next/headers', () => ({`,
      `  cookies: ${mockApi}.fn(() => ({ get: ${mockApi}.fn(() => undefined) })),`,
      `  headers: ${mockApi}.fn(() => ({ get: ${mockApi}.fn(() => null) })),`,
      `}))`
    )
  }
  ...
  if (a.hasCache) {
    lines.push(
      "",
      "NEXT/CACHE MOCK (no-op):",
      `${mockApi}.mock('next/cache', () => ({`,
      `  revalidatePath: ${mockApi}.fn(),`,
      `  revalidateTag: ${mockApi}.fn(),`,
      `  unstable_cache: ${mockApi}.fn((fn: any) => fn),`,
      `}))`
    )
  }
  ...
  if (a.clientModules.length > 0) {
    lines.push(
      "",
      "CLIENT MODULE MOCKS (boundary isolation required):"
    )

    for (const p of a.clientModules) {
      lines.push(
        `${mockApi}.mock('${p}', () => ({ /* export stubs */ }))`
      )
    }
  }
  ...
  if (a.sessionProviders.length > 0) {
    ...
    lines.push(
      ...
      "AUTH / SESSION MOCK:",
      `${mockApi}.mock('${target}', () => ({`,
      `  useSession: ${mockApi}.fn(() => ({`,
      `    data: { user: { id: 'test-user', email: 'test@example.com' } },`,
      `    status: 'authenticated'`,
      `  })),`,
      `  getSession: ${mockApi}.fn(() => Promise.resolve({ user: { id: 'test-user' } }))`,
      `}))`
    )
  }
  ...
  lines.push(
    "",
    "FINAL RULE:",
    "If imports still fail, the mock path does NOT match the runtime import string exactly (character-for-character)."
  )

  return lines.join("\n")
}

export function detectNextJsImportError(
  errorOutput: string,
  mockApi = 'vi'
): string | null {
  ...
  if (
    /Objects are not valid as a React child/i.test(errorOutput) ||
  ...
  if (
    /Server Action/i.test(errorOutput) ||
  ...
  if (isServerOnly) {
    lines.push(
      "server-only cannot be resolved in test runtime.",
      "Fix must be in test config (module alias to empty stub)."
    )
  }

  if (isAlias) {
    lines.push(
      "TS path alias '@/' is not resolved in test environment.",
      "Use relative mock paths instead of alias."
    )
  }

  if (isClientBoundary || isServerBoundary) {
    lines.push(
      "Next.js boundary module detected.",
