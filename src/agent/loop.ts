...
import type { FixOptions } from './fix-loop.js'
...
export interface LoopOptions {
  ...
  verbose: boolean
  targetFile?: string
  // Absolute path to a directory the run is scoped to (`lacuna generate <dir>`). Restricts
  // discovery + the coverage run to that subtree and implies improve-existing behavior.
  scopeDir?: string
  // Improve existing below-threshold tests (not just create tests for untested files).
  // Implied by scopeDir; also settable repo-wide via `lacuna generate --improve`.
  improve?: boolean
  // Patch-coverage mode (`lacuna generate @diff[:<ref>]`): target only the lines this branch
  // changed vs the base ref. '' = resolve the default base (origin/HEAD → main/master → HEAD~1);
  // undefined = off. Distinct from scopeDir — this scopes by diff, not by directory.
  diffRef?: string
  workers?: number
  fresh?: boolean
  // When a gap's generate attempts exhaust maxIterations, hand the best-collecting attempt
  // straight to the fix specialist (buildFixPrompt's fuller hint suite — hook/service/mock-call
  // hints, the exact runtime error, the real mocks file) for its OWN maxIterations, instead of
  // just giving up and requiring a separate manual `lacuna fix` pass. Default true; `lacuna
  // generate --no-fix-on-failure` disables it. Mirrors fix's existing `regenerateOnFailure` in
  // spirit (fix already falls back to generate-from-scratch on ITS exhaustion — this is the
  // same idea in the other direction) and reuses the SAME worker slot, so under `--workers N`
  // it happens interleaved with other files' generation, not as a separate serial pass.
  fixOnFailure?: boolean
  log: (msg: string) => void
  // Structured per-worker phase stream (WorkerState). Unset by the CLI (which renders its own
  // WorkerDisplay / verbose log); an embedding host (VS Code extension) sets this to consume
  // progress as data instead of scraping stdout. When set, it is called in addition to the
  // internal WorkerDisplay, and it flips the single-file / serial paths into structured mode
  // (suppressing the CLI's `!onStatus` verbose logging) exactly as the worker pool already is.
  onStatus?: (state: WorkerState) => void
  // Structured non-phase event stream (memory usage, and future cost/token readouts). Also
  // unset by the CLI. See lib/events.ts.
  onEvent?: LacunaEventHandler
  // Cooperative cancellation, checked at each file-iteration boundary. Returning false stops the
  // loop from picking up further files (in-flight files finish). Unset by the CLI (never cancels);
  // an embedding host wires it to a "stop" control — the responsible answer to the cost incident
  // that motivated the disclosure model.
  shouldContinue?: () => boolean
  // Companion to shouldContinue for INSTANT stop: aborts the in-flight model request (not just
  // between attempts), so "Stop" doesn't wait out a running generation. Unset by the CLI.
...
export interface LoopResult {
  ...
  // Patch-coverage mode only: covered changed executable lines / total changed executable
  // lines (Codecov semantics), before and after generation. Undefined outside @diff runs.
  patchCoverageBefore?: number
  patchCoverageAfter?: number
  diffBase?: string      // resolved base ref the diff was taken against (e.g. origin/main)
  errors: string[]
  // How many files exhausted generate's own retries and were handed to the fix specialist
  // (fixOnFailure), and how many of those the specialist actually recovered. Printed as a
  // persistent end-of-run line — the per-file 'fixing' WorkerState is transient (overwritten
  // the moment that worker picks up its next file), so this is the only lasting evidence the
  // handoff ran at all on a multi-worker run.
  fixHandoffs?: number
...
export async function processGap(
...
): Promise<{ success: boolean; error?: string; testCode?: string; fixHandoffAttempted?: boolean }> {
  ...
  let fixMemoryHint: string | null = null
  let stallRetries = 0
  ...
  let bestRunResult: RunResult | null = null
  ...
  for (let attempt = 1; attempt <= effectiveMax; attempt++) {
    ...
    if (generator.isPatch && !patchBase) {
      ...
      if (!onStatus) log(chalk.yellow(`  Patch mode used for a new file — retrying with full-file output...`))
      onStatus?.({ phase: 'retrying', file: shortPath, attempt, max: effectiveMax } as WorkerState)
      continue
    ...
    if (genSubject && !referencesSubject(testCode, genSubject)) {
      ...
      if (!onStatus) log(chalk.red(`  ⚠ Test doesn't test its subject (${genSubject}) — rejected, retrying...`))
      onStatus?.({ phase: 'retrying', file: shortPath, attempt, max: effectiveMax } as WorkerState)
      continue
    }
    ...
    if (runResult.success) {
      ...
      if (!dryRun) {
        ...
        if (routed && !onStatus) log(chalk.dim(`  ↳ DOM-free — routed to the node environment (skips jsdom startup).`))
      }
      onStatus?.({ phase: 'passed', file: shortPath })
    ...
    if (attempt === 1) {
      ...
      if (!onStatus) log(chalk.red(`  Fix caused regression: ${firstPassCount} → ${passCount} passing (attempt ${attempt}/${effectiveMax})`))
    } else {
      consecutivePatchFailures = 0
      lastError = extracted + (buildFailingTestChecklist(rawRunOutput) ?? '')
      if (!onStatus) log(chalk.red(`  Tests failed (attempt ${attempt}/${effectiveMax})`))
    }
    // Surface the mocks-patch failure alongside whatever the test run itself reported — see the
    // identical note in fix-loop.ts.
    if (mocksPatchFailureNote) lastError = `${lastError}\n\n${mocksPatchFailureNote}`
  ...
  if (originalTestContent === null) {
    ...
    if (bestCode !== null) { await writeFile(context.suggestedTestFile, bestCode, 'utf-8'); keepHint(); keptBestOnDisk = true }
    else {
  ...
  if (fixOnFailure && !dryRun && (keptBestOnDisk || hasBrokenNewFileOnDisk)) {
    ...
    const fixOptions: FixOptions = { config: { ...config, maxIterations: Math.max(1, Math.ceil(config.maxIterations / 2)) }, env, cwd, dryRun, verbose, log }
    ...
    if (fixResult.success) {
      ...
      if (!onStatus) log(chalk.green(`  ✓ Fix specialist recovered ${rel}.`))
    ...
    if (!onStatus) log(chalk.yellow(`  Fix specialist could not recover ${rel} either.`))
    return {
      success: false,
...
async function runWorkerPool(
  ...
  workerCount: number,
  projectMemory: string | null,
): Promise<{ filesProcessed: number; testsWritten: number; errors: string[]; fixHandoffs: number; fixHandoffRecovered: number }> {
  ...
  const workers = Array.from({ length: workerCount }, async (_, wi) => {
    ...
    while (true) {
      ...
      if (result.fixHandoffAttempted) {
        fixHandoffs++
        if (result.success) fixHandoffRecovered++
      }
...
export async function runAgentLoop(options: LoopOptions): Promise<LoopResult> {
  ...
  let diffScope: DiffScope | null = null
  if (diffMode) {
  ...
  const gaps = await filterTestableGaps(
    extractGaps(report, diffMode ? 101 : config.threshold),
    config.ignore,
    { includeExisting: improveExisting, cwd },
  )
  ...
  let diffGaps = gaps
  let patchCoverageBefore: number | undefined
  if (diffScope) {
    ...
    patchCoverageBefore = computePatchCoverage(
      report, diffScope.changed, cwd, filesOutsideReport(diffGaps, report, cwd),
    ).pct
  }
...
function mergeReportHits(base: CoverageReport, incr: CoverageReport, cwd: string): CoverageReport {
  const abs = (p: string) => (p.startsWith('/') ? p : join(cwd, p))
  const byPath = new Map<string, FileCoverage>()
  for (const f of base.files) byPath.set(abs(f.path), { ...f, lines: f.lines.map((l) => ({ ...l })) })
  for (const f of incr.files) {
    const key = abs(f.path)
    const existing = byPath.get(key)
    if (!existing) { byPath.set(key, { ...f, lines: f.lines.map((l) => ({ ...l })) }); continue }
    const hitByLine = new Map<number, number>(existing.lines.map((l) => [l.line, l.hit]))
    for (const l of f.lines) hitByLine.set(l.line, Math.max(hitByLine.get(l.line) ?? 0, l.hit))
...
function filesOutsideReport(gaps: CoverageGap[], report: CoverageReport, cwd: string): Set<string> {
  ...
  for (const g of gaps) {
    ...
    if (!reportPaths.has(abs)) outside.add(abs)
  }
  return outside
}
