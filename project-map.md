...
| `prompts/react-native.ts` | `detectReactNative(packageDeps)`. `buildReactNativeGuidance()` — RNTL v14 rules: render() is async (must await), fireEvent is async (must await), loading state controlled-Promise pattern, stateless icon stubs use plain `() => null` (not `jest.fn()`), compound component mocks use static factory, button queries (`getAllByText[n]` not `getAllByRole('button')`), disabled TouchableOpacity, test cascade. **Mock accuracy rules**: mock shape must match hook destructure exactly (missing keys are silent `undefined`), service method names must be copied verbatim from the component source, mock data helpers must include all array fields accessed by the component. **Dynamic text**: compute expected string from mock state (e.g. `activeStatus: 'open'` → `'No open draws right now'`). **Toast vs screen text**: if catch block calls `showToast`, assert on `showToast` not `screen.getByText` — use `mockRejectedValue` not `{ success: false }`. **Icon-only buttons**: can't be queried without testID, skip or add testID. **useFocusEffect**: mock as no-op to pin view at initial useState. Infrastructure mocks: Reanimated, AsyncStorage, safe area context, vector icons, React Navigation, Zustand, Redux. `detectRntlErrors(errorOutput)` — fires ⚠️ warnings for common RNTL errors. |
