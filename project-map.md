...
| `prompts.ts` | All prompt builders. **`buildSystemPrompt(env)`** — fully conditional on `isJS`, `isTS`, `isVitest`, `isJSRunner`. **Thinking template** — 5 steps including MOCK AUDIT (a–h). **Vitest global-spy rule** + **duplicate vi.mock rule**. `buildGeneratePrompt` / `buildFixPrompt` — inject Next.js/React Native/Vue guidance, **mock module inventory** (see below). `buildPollutionFixPrompt`. `detectTypeScriptErrors`. `detectNextJsImportError`. `buildRetryPrompt`. **Mock inventory**: `parseMockInventory(code)` scans every `vi.mock()` call, extracts line number + top-level factory entries as `key(Variable)` pairs — e.g. `ValidationError(MockValidationError)`. When key and variable follow the obvious `mock`-prefix convention the parenthetical is omitted; when they differ (e.g. `MockValidationError` with capital M, or `useSessions(mockUseSessions)`) the mapping is shown explicitly so the AI knows which export to import. Both prompt builders show this inventory before the raw/filtered mock file. 3 editing rules — (1) module already in inventory: update existing block, NEVER append a second vi.mock(); (2) new export variable: add near same-domain exports, reset in EXISTING beforeEach; (3) new module not in inventory: append at end before final beforeEach. `parseMockExports(code)` — flat list of exported names for the "Available exports" line. |
