...
| `prompts.ts` | All prompt builders. `buildSystemPrompt(env)` — senior QA engineer persona; rules 11–13 require `<thinking>` + `<code_output>` XML output structure. `buildGeneratePrompt(args)` — includes projectMemory, deps, paths, setup file, mocks (with `parseMockExports` inventory), source skeleton + import path, existing tests, uncovered items, network mocking guidance. `buildFixPrompt(args)` — repair-focused; adds `detectRealRequestInError` warning if error output shows a real HTTP call; adds wrong-mock warning if test has `vi.mock('axios')` but source uses `axios.create()`. `buildRetryPrompt(failureOutput, failedAttempts?)` — prepends negative-constraint block listing past hypotheses and their failure reasons before the error output. `parseMockExports(code)` — extracts exported names from mocks file. `analyzeNetworkDeps(sourceCode)` — detects axios/fetch usage and API module imports (directory and file-level patterns). `buildNetworkMockingGuidance(analysis, file)` — returns targeted mocking instructions. `detectRealRequestInError(errorOutput)` — scans for real URLs, 4xx/5xx status codes, ECONNREFUSED. Exports `FailedAttempt` interface. |
...
| File | Purpose |
|------|---------|
| `config.ts` | `loadConfig()` — cosmiconfig loader. Zod schema with defaults. Config fields: `testRunner`, `coverageFormat`, `coverageDir` (default `coverage`), `sourceDir` (default `src`), `threshold` (80), `maxIterations` (3), `coverageTimeout` (300s), `maxTokens` (16000), `ignore[]`, `mocksFile`, `setupFile`, `provider`, `model`, `baseURL`, `apiKeyEnv` |
...
| File | Purpose |
|------|---------|
| `types.ts` | `ModelProvider` interface (`generate(messages, system, onToken?, maxTokens?, temperature?) → Promise<string>`), `ChatMessage`, `ProviderPreset`. `PRESETS` map: claude, claude-opus, deepseek, deepseek-r1, gpt-4o, groq, openrouter, ollama, lm-studio, custom. |
| `anthropic.ts` | `AnthropicProvider` — uses `@anthropic-ai/sdk`. Streams tokens if `onToken` provided. `max_tokens` from param (default 16000). `stop_sequences: ['</code_output>']`. `temperature` passed as optional spread. |
| `openai-compatible.ts` | `OpenAICompatibleProvider` — uses `openai` SDK with custom `baseURL`. Covers DeepSeek, Groq, OpenRouter, Ollama, LM Studio, GPT-4o. `max_tokens` from param (default 16000). `stop: ['</code_output>']`. `temperature` optional spread. |
...
- **Fix cache**: `runFixLoop` writes discovered failing files to `.lacuna-fix-cache.json` after a full suite run. On the next invocation (within 5 minutes, without `--fresh`), the cache is reused and the suite is skipped. Not used in `--file` mode or when `--fresh` is passed.
