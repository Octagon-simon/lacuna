...
| `init.ts` | `lacuna init` | Interactive wizard using `@inquirer/prompts`. Writes `.lacuna.json`. **Runs from any subdirectory** — `findProjectRoot()` walks up to nearest `package.json`. Asks user for `sourceDir` explicitly. Detects React + TypeScript + Next.js from `package.json`. For React/Next.js: installs `jsdom` + `@testing-library/react/jest-dom/user-event`; uses async for-loop (not `.filter`) to check which are missing. **No `@vitejs/plugin-react`** — Vitest uses esbuild for JSX natively. For Next.js: setup file goes to `test/setup.ts` at root (NOT inside `app/` which is App Router); vitest.config.ts reads `@` alias target from `tsconfig.json` `compilerOptions.paths["@/*"]` via `resolveAtAlias()`; setup file includes global `vi.mock()` for `next/navigation`, `next/headers`, `next/cache`. Config files use `resolve(cwd, ...)` not `join`. |
