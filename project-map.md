...
| `prompts.ts` | All prompt builders. `buildSystemPrompt(env)` — senior QA engineer persona; rules 11–13 require `<thinking>` + `<code_output>` XML output structure. `buildGeneratePrompt(args)` — includes projectMemory, deps, tsconfig, **TYPE DEFINITIONS** (if present: `interface`/`type`/`enum` declarations from imported files with instruction "use these exact shapes, do NOT invent properties or guess types"), setup file, mocks (with `parseMockExports` inventory), source skeleton + import path, existing tests, uncovered items, network mocking guidance. `buildFixPrompt(args)` — same TYPE DEFINITIONS section; repair-focused; adds `detectRealRequestInError` warning if error output shows a real HTTP call; adds wrong-mock warning if test has `vi.mock('axios')` but source uses `axios.create()`. `buildRetryPrompt(failureOutput, failedAttempts?)` — prepends negative-constraint block listing past hypotheses and their failure reasons before the error output. `parseMockExports(code)` — extracts exported names from mocks file. `analyzeNetworkDeps(sourceCode)` — detects axios/fetch usage and API module imports (directory and file-level patterns). `buildNetworkMockingGuidance(analysis, file)` — returns targeted mocking instructions. `detectRealRequestInError(errorOutput)` — scans for real URLs, 4xx/5xx status codes, ECONNREFUSED. Exports `FailedAttempt` interface. |
