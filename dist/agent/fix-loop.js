...
async function formatFileVerified(absFile, cwd, config, env) {
    if (!config.format)
        return; // formatFile would no-op; skip the whole (costly) verify
    ...
    try {
        ...
        if (!res.success)
            await writeFile(absFile, green, 'utf-8');
    }
 ...
 */
export async function discoverFailingTests(config, env, cwd, scopeDir, onLine, signal) {
    ...
    if (result.timedOut) {
        throw new DiscoverFailingError(`Test suite timed out after ${config.coverageTimeout}s — raise coverageTimeout in .lacuna.json.`);
    }
    if (result.success)
        return { failing: [], allPassing: true };
    const raw = result.stdout + result.stderr;
    const configConflict = detectJestConfigConflict(raw);
    if (configConflict)
        throw new DiscoverFailingError(`Jest never ran any tests — nothing was discovered to fix.\n\n${configConflict}`);
    const validationError = detectJestValidationError(raw);
    if (validationError)
        throw new DiscoverFailingError(validationError);
    ...
    for (const f of parseFailingTestFiles(raw, env.testRunner)) {
        ...
        if (!abs || !abs.startsWith(cwd) || abs.includes('node_modules'))
            continue;
...
export function extractRelativeImportSpecifiers(testCode) {
    const specifiers = [];
    const re = /import\s+(?:[\w*\s{},]+)\s+from\s+['"](\.[^'"]+)['"]/g;
    let m;
    while ((m = re.exec(testCode)))
        specifiers.push(m[1]);
    return specifiers;
}
export async function findSourceFile(testFilePath, cwd, configSourceDirs = 'src', testCode) {
...
export async function fixFile(testFilePath, options, generator, onStatus, projectMemory, 
...
precomputedFirstRun) {
    ...
    if (firstRun.success) {
        ...
        if (typeErrorsAtStart === TYPECHECK_INCONCLUSIVE) {
            ...
            if (!onStatus)
                log(chalk.red(`  ⚠ Could not type-check ${shortPath} (tsc did not complete) — leaving as unresolved.`));
            onStatus?.({ phase: 'failed', file: shortPath });
        ...
        if (!typeErrorsAtStart) {
            ...
            const fixable = leaked && leakLooksTestFixable(leakCheckTestCode, await readFile((await findSourceFile(testFilePath, cwd, config.sourceDir, leakCheckTestCode)) ?? '', 'utf-8').catch(() => null));
            if (!fixable) {
                if (!onStatus)
                    log(chalk.dim(leaked
            ...
            if (!onStatus)
                log(chalk.yellow('  Tests pass but a timer/handle leaked — adding cleanup.'));
        }
        else {
            if (!onStatus)
    ...
    for (let attempt = 1; attempt <= effectiveMax; attempt++) {
        ...
        if (originalTestsSubject && subject && !referencesSubject(testFileContent, subject)) {
            ...
            if (!onStatus)
                log(chalk.red(`  ⚠ Rewrite abandoned the subject (${subject}) — rejected, retrying...`));
            onStatus?.({ phase: 'retrying', file: shortPath, attempt, max: effectiveMax });
            continue;
        }
        ...
        if (result.success) {
            ...
            if (typeErrors === TYPECHECK_INCONCLUSIVE) {
                ...
                if (!onStatus)
                    log(chalk.red(`  ⚠ Could not verify types (tsc did not complete) — leaving as unresolved.`));
                await writeFile(absTestPath, bestCode, 'utf-8').catch(() => { });
                onStatus?.({ phase: 'failed', file: shortPath });
            ...
            if (config.memory.enabled)
                await recordFixMemory('success', testFileContent);
            return { success: true };
        }
        ...
        if (crashSignature) {
            ...
            if (!onStatus)
                log(chalk.red(`  Fix caused regression: ${baselinePassCount} → ${currentPassCount} passing (attempt ${attempt}/${effectiveMax})`));
        }
            ...
            if (!onStatus)
                log(chalk.red(`  All ${currentPassCount} tests pass but the run failed on unhandled errors (attempt ${attempt}/${effectiveMax})`));
        }
        else {
            consecutivePatchFailures = 0;
            errorOutput = extracted + (buildFailingTestChecklist(rawRunOutput) ?? '');
            if (!onStatus)
                log(chalk.red(`  Still failing (attempt ${attempt}/${effectiveMax})`));
        }
    ...
    if (reachedGreen && !firstRun.success) {
        if (!onStatus)
            log(chalk.yellow(`  ⚠ Tests pass — kept the fix. Couldn't auto-clear every type error in ${effectiveMax} attempts; re-run fix to try the rest.`));
        onStatus?.({ phase: 'passed', file: shortPath });
        if (config.memory.enabled)
            await recordFixMemory('success', bestCode);
    ...
    onStatus?.({ phase: 'failed', file: shortPath });
    if (config.memory.enabled)
        await recordFixMemory('failure', bestCode);
...
async function discoverTestFiles(cwd, env, scopeDir) {
...
async function regenerateFile(testFilePath, options, onStatus, projectMemory, baselinePassCount = 0) {
    ...
    const cappedOptions = { ...options, config: { ...options.config, maxIterations: Math.max(1, Math.ceil(options.config.maxIterations / 2)) } };
    ...
    if (result.success) {
        ...
        if (regenRun.timedOut || regenRun.aborted)
            return { success: true };
        ...
        while (true) {
            ...
            if (result.success) {
                if (result.skipped) {
                    ...
                    if (!options.dryRun)
                        await formatFileVerified(absFile, options.cwd, options.config, options.env);
                }
                else {
                    filesFixed++;
                    if (!options.dryRun)
...
export async function runFixLoop(options) {
    ...
    if (options.types && !options.targetFile) {
        ...
        if (fileResult.success) {
            ...
            const leakFixable = leaked && leakLooksTestFixable(leakCheckTargetCode, await readFile((await findSourceFile(absTarget, cwd, config.sourceDir, leakCheckTargetCode)) ?? '', 'utf-8').catch(() => null));
        ...
        if (useCached) {
            ...
            const searchRoot = scopeDir ?? cwd;
            const resolved = new Set();
            const unresolvedParsed = [];
            for (const f of parsedFiles) {
                const abs = await resolveReportedTestPath(f, cwd, searchRoot);
                if (!abs) {
                    unresolvedParsed.push(f);
                    continue;
                }
            ...
            if (expectedFailedCount !== null && failingFiles.length < expectedFailedCount) {
                ...
                await debugWrite(debugFile, 'DISCOVERY UNDERCOUNT', `parsedFiles (${parsedFiles.length}): ${JSON.stringify(parsedFiles)}\n` +
                    `unresolvedParsed (${unresolvedParsed.length}): ${JSON.stringify(unresolvedParsed)}\n` +
                    `resolved (${resolved.size}): ${JSON.stringify([...resolved])}\n\n` +
                    `RAW OUTPUT:\n${rawDiscoveryOutput}`, true);
        ...
        for (const file of failingFiles) {
            ...
            if (result.success) {
                if (result.skipped) {
                    ...
                    stillFailingFiles.push(file);
                    if (!options.dryRun)
                        await formatFileVerified(absFile, cwd, config, env);
                }
                else {
                    filesFixed++;
                    if (!options.dryRun)
