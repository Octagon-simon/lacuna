    ...
    for (let attempt = 1; attempt <= config.maxIterations; attempt++) {
        ...
        if (result.success) {
            ...
            if (typeErrors) {
                ...
                if (!onStatus)
                    log(chalk.yellow(`  ⚠ Tests now pass — keeping the fix. Type errors remain; run \`lacuna fix --file ${shortPath}\` to clean them up.`));
                onStatus?.({ phase: 'passed', file: shortPath });
                return { success: true };
            }
            if (!onStatus)
        ...
        if (structureBroken) {
            ...
            if (!onStatus)
                log(chalk.red(`  All ${currentPassCount} tests pass but the run failed on unhandled errors (attempt ${attempt}/${config.maxIterations})`));
        }
        else {
