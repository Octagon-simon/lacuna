 ...
 */
export async function discoverFailingTests(config, env, cwd, scopeDir, onLine) {
        ...
        while (true) {
            ...
            if (result.success) {
                if (result.skipped) {
                    ...
                    if (!options.dryRun)
                        await formatFile(absFile, options.cwd, { enabled: options.config.format, env: options.env });
                }
                else {
                    filesFixed++;
                    if (!options.dryRun)
