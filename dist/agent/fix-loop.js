...
precomputedFirstRun) {
    ...
    if (firstRun.success) {
        ...
        if (!typeErrorsAtStart) {
            ...
            const fixable = leaked && leakLooksTestFixable(await readFile(absTestPath, 'utf-8').catch(() => ''), await readFile((await findSourceFile(testFilePath, cwd, config.sourceDir)) ?? '', 'utf-8').catch(() => null));
            if (!fixable) {
                if (!onStatus)
...
export async function runFixLoop(options) {
    ...
    if (options.types && !options.targetFile) {
        ...
        if (fileResult.success) {
            ...
            const leakFixable = leaked && leakLooksTestFixable(await readFile(absTarget, 'utf-8').catch(() => ''), await readFile((await findSourceFile(absTarget, cwd, config.sourceDir)) ?? '', 'utf-8').catch(() => null));
