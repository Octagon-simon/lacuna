...
export class TestGenerator {
    ...
    async generate(context, gap, projectMemory) {
        ...
        const response = await this.provider.generate(this.history, buildSystemPrompt(this.env), this.rawOnToken ? codeOnlyStream(this.rawOnToken) : undefined, estimateMaxTokens(context.sourceCode, this.maxTokens), GENERATE_TEMPERATURE);
    ...
    async fix(args) {
        ...
        const response = await this.provider.generate(this.history, buildSystemPrompt(this.env), this.rawOnToken ? codeOnlyStream(this.rawOnToken) : undefined, estimateMaxTokens(args.sourceCode, this.maxTokens), GENERATE_TEMPERATURE);
        const { hypothesis, code, truncated } = parseStructuredResponse(response);
        this.lastHypothesis = hypothesis;
        this.previousCodes.push(normalizeCode(code));
        this.history.push({ role: 'assistant', content: response });
