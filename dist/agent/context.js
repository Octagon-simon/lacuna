...
async function resolvePackageExport(pkgRoot, subpath) {
    let pkg;
    try {
        pkg = JSON.parse(await readFile(join(pkgRoot, 'package.json'), 'utf-8'));
    }
    catch {
        return null;
    }
    ...
    if (!target && !subpath)
        target = pkg.types ?? pkg.typings ?? pkg.main;
    if (!target)
        return null;
...
async function resolveViaPackageExports(importPath, aliases, cwd) {
    for (const [pattern, targets] of Object.entries(aliases)) {
        const aliasPrefix = pattern.replace(/\*$/, '');
        if (!importPath.startsWith(aliasPrefix))
            continue;
...
function parseNamedList(braceContent) {
    return braceContent
        .split(',')
        .map((s) => s.trim())
        .filter(Boolean)
...
function extractModuleSpecifiersWithNames(code) {
    const specs = [];
...
function extractTypeDeclarations(code, neededNames = null) {
    ...
    while (i < lines.length) {
        ...
        if (declMatch) {
            if (neededNames && !neededNames.includes(declMatch[1])) {
                i++;
                continue;
            }
            const block = [line];
...
export async function collectTypeDefinitions(sourceCode, absoluteSourcePath, cwd) {
    ...
    const visitKey = (file, names) => `${file}::${names ? names.slice().sort().join(',') : '*'}`;
    const blocks = [];
    let totalChars = 0;
    ...
    async function visit(specifier, fromAbsolutePath, neededNames, arrivedViaWildcard, branchBlocksStart, branchCharsStart) {
        if (blocks.length >= MAX_TYPE_FILES || totalChars >= MAX_TYPE_CHARS)
            return;
        if (blocks.length - branchBlocksStart >= perImportFileCap)
            return;
        if (totalChars - branchCharsStart >= perImportCharCap)
            return;
        const base = resolveLocalImport(specifier, fromAbsolutePath, cwd, aliases);
        let file = base ? await resolveToFile(base) : null;
        if (!file)
            file = await resolveViaPackageExports(specifier, aliases, cwd);
        if (!file || file === absoluteSourcePath)
            return;
        ...
        for (const { path: nextSpecifier, names } of extractModuleSpecifiersWithNames(content)) {
            ...
            if (totalChars - branchCharsStart >= perImportCharCap)
                break;
            if (neededNames && names && !names.some((n) => neededNames.includes(n)))
                continue;
...
export async function collectLocalImportPaths(sourceCode, absoluteSourcePath, absoluteTestFilePath, cwd) {
    ...
    for (const m of sourceCode.matchAll(IMPORT_RE)) {
        ...
        let file = base ? await resolveToFile(base) : null;
        if (!file)
            file = await resolveViaPackageExports(importPath, aliases, cwd);
