...
async function inferTestFilePath(sourceFile, cwd, env) {
...
async function findExistingTestFile(sourceFile, cwd) {
    const ext = extname(sourceFile);
    const base = basename(sourceFile, ext);
    const dir = dirname(sourceFile);
    const candidates = [
...
export async function buildFileContext(sourceFilePath, cwd, env, config) {
    ...
    const suggestedTestFile = existingTestFile ?? join(cwd, await inferTestFilePath(sourceFilePath, cwd, env));
