...
export default class Generate extends Command {
    ...
    async run() {
        ...
        if (diffHeader)
            this.log(`${chalk.dim('Scope:')}      ${chalk.cyan(diffHeader)}`);
        else if (scopeDir)
            this.log(`${chalk.dim('Scope:')}      ${args.path} ${chalk.dim('(create + improve)')}`);
        else if (targetFile)
            this.log(`${chalk.dim('Target:')}     ${targetFile}`);
        else if (improve)
            this.log(`${chalk.dim('Mode:')}       ${chalk.cyan('improve')} ${chalk.dim('(extend existing below-threshold tests)')}`);
