...
function isCleanRunnerScript(script, runner) {
    if (/[&|;]/.test(script))
        return false;
    const s = script.trim().replace(/^npx\s+/, '');
    if (runner === 'vitest')
        return /^vitest(\s+run)?(\s|$)/.test(s);
    if (runner === 'jest')
        return /^jest(\s|$)/.test(s);
    return false;
}
...
export async function findTestRoot(fromDir, repoRoot, runner) {
    let dir = fromDir;
    ...
    while (true) {
        ...
        if (await exists(pkgPath)) {
            let script;
            try {
                script = (JSON.parse(await readFile(pkgPath, 'utf-8')).scripts ?? {}).test;
            }
            catch { /* ignore */ }
            if (script && isCleanRunnerScript(script, runner))
                return { cwd: dir, npmTest: true };
            if (!fallbackCwd)
...
export async function resolveScopeTestRun(env, absDir, repoRoot) {
    ...
    if (npmTest)
        return { command: rel ? `npm test -- ${npmTestArg(env, rel)}` : 'npm test', cwd };
...
export async function resolveIncrementalCoverageRun(env, absTestFile, absSourceFile, repoRoot, outDir) {
    if (env.testRunner !== 'vitest' && env.testRunner !== 'jest')
        return null;
