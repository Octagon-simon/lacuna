    ...
    if (input.type === 'generate' && input.generate) {
        ...
        if (r.hasCoverage) {
            ...
            if (r.patchCoverageAfter !== undefined) {
                ...
                console.log(`  Patch coverage  : ${chalk.dim(pBefore.toFixed(1) + '%')} → ${pColor(r.patchCoverageAfter.toFixed(1) + '%')}${r.diffBase ? chalk.dim(` (diff vs ${r.diffBase})`) : ''}`);
            }
            const gatePct = r.patchCoverageAfter ?? r.coverageAfter;
            console.log(`  Threshold       : ${threshold}%  ${gatePct >= threshold ? chalk.green('PASS') : chalk.red('FAIL')}`);
        }
...
export function buildMarkdownReport(input) {
    ...
    if (input.type === 'generate' && input.generate) {
        ...
        if (r.patchCoverageAfter !== undefined) {
            ...
            if (r.diffBase)
                lines.push(`| Diff base | \`${r.diffBase}\` |`);
        }
        lines.push(`| Threshold | ${threshold}% |`);
    ...
    lines.push('');
    lines.push(`> Generated by [lacuna](https://github.com/Octagon-simon/lacuna)`);
    return lines.join('\n');
}
// ─── Exit codes ──────────────────────────────────────────────────────────────
export const EXIT = {
