...
async function findExistingConfig(cwd, names) {
...
function warnIfMissingLcovCoverage(configPath, configText, coverageDir, kind, log) {
...
async function warnIfMissingAliasMapper(configPath, configText, cwd, log) {
    if (/moduleNameMapper/.test(configText))
        return;
    ...
    let paths;
    try {
        paths = JSON.parse(cleaned).compilerOptions?.paths;
    }
    catch {
        return;
    }
...
export async function readProjectMeta(cwd) {
    try {
...
function buildSetupFileContent(variant, runner, isExpo = false, hasReanimated = false, hasWorklets = false) {
...
export async function ensureTestRunnerSetup(runner, sourceDir, cwd, log, yes = false, 
...
confirmInstall) {
    ...
    if (runner === 'vitest') {
        ...
        if (existing) {
            ...
            if (meta.isReactNative)
                await warnIfMissingAliasMapper(existing, existingText, cwd, log);
        }
        else {
            const setupLine = createdSetupFile
 ...
 */
async function resolveRunner(cwd, requested, log) {
