...
function hookReturnsField(sourceCode, field) {
    ...
    for (let i = sourceCode.indexOf('return'); i !== -1; i = sourceCode.indexOf('return', i + 6)) {
        ...
        if (braceStart === -1 || braceStart - i > 12)
            continue; // not `return {`
...
export function buildStaleHookStateHint(errorOutput, testCode, sourceCode) {
    if (!errorOutput)
        return null;
    const fieldMatch = STALE_CURRENT_RE.exec(errorOutput);
    if (!fieldMatch || !STALE_RECEIVED_RE.test(errorOutput))
        return null;
    const field = fieldMatch[1];
    const returned = sourceCode ? hookReturnsField(sourceCode, field) : true; // assume returned if unknown
    if (!returned) {
        return (`STALE HOOK FIELD — \`${field}\` IS NOT RETURNED BY THE HOOK: the test asserts ` +
    ...
    return (`STALE HOOK STATE — MISSING act(): the test reads \`result.current.${field}\` right after a ` +
...
//# sourceMappingURL=stale-hook-state-hints.js.map